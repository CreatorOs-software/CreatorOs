  ...
  async findById(supabase: SupabaseClient, id: string, agencyId: string) {
    const [brandRes, contactsRes, dealsRes, anfrageRes, creatorsRes] = await Promise.all([
      ...
      supabase
        .from("deals")
        .select(
          "id, title, budget, status, deadline, campaign_type, usage_rights, exclusivity, payment_items, created_at, creator_id, creators(id, full_name, initials, avatar_config)",
        )
        .eq("brand_id", id)
        .eq("agency_id", agencyId)
      ...
      supabase
        .from("anfragen")
        .select(
          "id, format, budget_requested, status, rejection_reason, notes, created_at, creator_id, creators(id, full_name, initials, avatar_config)",
        )
        .eq("brand_id", id)
        .eq("agency_id", agencyId)
        .order("created_at", { ascending: false }),
