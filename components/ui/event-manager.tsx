...
const TYPE_CONFIG: Record<
  EventType,
  { label: string; bg: string; border: string; dot: string; pill: string }
> = {
  shoot: {
    label: "Shoot",
    bg: "bg-purple-500",
              ...
              <div
                ...
                onDrop={(e) => {
                  ...
                  const hour = Math.max(
                    ...
                    Math.min(
                      23,
                      Math.floor((e.clientY - rect.top) / HOUR_HEIGHT),
                    ),
                  );
                  onDrop(day, hour);
                }}
      ...
      <div className="flex flex-1 overflow-auto">
        ...
        <div
          ...
          onDrop={(e) => {
            ...
            const hour = Math.max(
              0,
              Math.min(23, Math.floor((e.clientY - rect.top) / HOUR_HEIGHT)),
            );
...
function ListView({
  ...
  const groups = useMemo(() => {
    ...
    for (const ev of sorted) {
      ...
      if (!map.has(key)) {
        map.set(key, {
          label: ev.startTime.toLocaleDateString("de-DE", {
            weekday: "long",
            day: "numeric",
            month: "long",
            year: "numeric",
          ...
          <Card className="divide-y">
            {group.events.map((ev) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn("h-10 w-1 shrink-0 rounded-full", cfg.bg)}
                  />
...
function EventDialog({
  ...
  const editStart = selectedEvent
    ? splitISO(selectedEvent.startTime.toISOString())
    : { date: "", time: "09:00" };
  const editEnd = selectedEvent
    ? splitISO(selectedEvent.endTime.toISOString())
    : { date: "", time: "10:00" };
  ...
  const activeAttendees = isCreating
    ? form.attendee_ids
    : (selectedEvent?.attendeeIds ?? []);
        ...
        ev
          ? {
              ...
              startTime: editStartDate
                ? new Date(combineToISO(editStartDate, editStartTime))
                : ev.startTime,
              endTime: editEndDate
                ? new Date(combineToISO(editEndDate, editEndTime))
                : ev.endTime,
            }
          : null,
      );
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-2xl" aria-describedby={undefined}>
        <DialogHeader>
          <DialogTitle>
            {isCreating ? "Neues Event" : "Event bearbeiten"}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  isCreating
                    ...
                    : setSelectedEvent((ev) =>
                        ev ? { ...ev, title: e.target.value } : null,
                      )
                }
              />
            </div>
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  isCreating
                    ...
                    : setSelectedEvent((ev) =>
                        ev ? { ...ev, allDay: checked } : null,
                      )
                }
              />
              <Label className="cursor-pointer whitespace-nowrap text-sm">
                Ganztägig
              </Label>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Popover>
              <PopoverTrigger className="flex min-h-9 w-full items-center justify-between rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none hover:bg-muted/30 focus-visible:ring-1 focus-visible:ring-ring">
                {activeAttendees.length === 0 ? (
                  ...
                  <span className="flex flex-wrap gap-1">
                    {activeAttendees.map((id) => {
                      ...
                      if (!m) return null;
                      const abbr =
                        m.initials ?? getInitials(m.display_name ?? "");
                      return (
                        <span
                          key={id}
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <DatePicker
                  value={
                    isCreating ? form.start_date || null : editStartDate || null
                  }
                  onChange={(v) =>
                    isCreating
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <DatePicker
                  value={
                    isCreating ? form.end_date || null : editEndDate || null
                  }
                  onChange={(v) =>
                    isCreating
            ...
            <Select
              ...
              onValueChange={(v) =>
                isCreating
                  ...
                  : setSelectedEvent((ev) =>
                      ev ? { ...ev, recurrence: v as Recurrence } : null,
                    )
              }
            >
              <SelectTrigger>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select
                value={
                  isCreating ? form.type : (selectedEvent?.type ?? "internal")
                }
                onValueChange={(v) =>
                  isCreating
                    ? setForm((f) => ({ ...f, type: v as EventType }))
                    : setSelectedEvent((ev) =>
                        ev ? { ...ev, type: v as EventType } : null,
                      )
                }
              >
                <SelectTrigger>
            ...
            <div className="space-y-1.5">
              ...
              {(() => {
                ...
                const displayCreator = creators.find(
                  (c) => c.id === selectedId,
                );
                return (
                  <Select
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              value={
                isCreating ? form.location : (selectedEvent?.location ?? "")
              }
              onChange={(e) =>
                isCreating
                  ? setForm((f) => ({ ...f, location: e.target.value }))
                  : setSelectedEvent((ev) =>
                      ev ? { ...ev, location: e.target.value } : null,
                    )
              }
            />
          </div>
            ...
            <Textarea
              ...
              onChange={(e) =>
                isCreating
                  ...
                  : setSelectedEvent((ev) =>
                      ev ? { ...ev, notes: e.target.value } : null,
                    )
              }
            />
          </div>
...
export function EventManager() {
  ...
  const filteredEvents = useMemo(() => {
    return allEvents.filter((ev) => {
      ...
      if (filterCreator !== "all" && ev.creatorId !== filterCreator)
        return false;
      if (searchQuery) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (form.end_date) body.end_at = combineToISO(form.end_date, endTime);
        ...
        body.end_at = combineToISO(form.start_date, "23:59");
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      {/* ── Header ── */}
      <div className="flex shrink-0  items-center gap-3 sm:flex-row sm:items-start sm:justify-between">
